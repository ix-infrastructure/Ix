...
/**
 * Create an authenticated IxClient. Refreshes the auth token if it is
 * close to expiry (or already expired) before constructing the client,
 * so every command gets a valid Bearer token when talking to a remote
 * instance. Safe to call when no remote instance is active — the
 * refresh is a no-op and the client works against localhost as usual.
 */
export async function createClient(): Promise<import("../client/api.js").IxClient> {
