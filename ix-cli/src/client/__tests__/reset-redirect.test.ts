...
// Use standard localhost on every platform. Select the async branch separately
// so macOS does not need an extra loopback alias; fetch and its errors are real.
function remoteClient(endpoint: string) {
  const client = new IxClient(endpoint);
  vi.spyOn(client as unknown as { isLocalEndpoint(): boolean }, "isLocalEndpoint").mockReturnValue(false);
  return client;
}

async function fixture(reply: (request: Seen, response: ServerResponse) => void, host = "127.0.0.1") {
  const seen: Seen[] = [];
...
afterEach(async () => {
  try { await Promise.all(servers.splice(0).map(close)); }
...
describe.each(["reset", "resetCode"] as const)("%s never follows a reset redirect", method => {
  ...
  it("completes the direct asynchronous request and matching status", async () => {
    ...
    await expect(remoteClient(remote.endpoint)[method]()).resolves.toMatchObject({ ok: true });
  ...
  it("preserves unreachable classification when the initial connection is refused", async () => {
    ...
    const failure = await remoteClient(closed.endpoint)[method]().catch((error: unknown) => error);
  ...
  it.each([307, 308])("does not replay the POST after a %s start response", async status => {
    ...
    await expect(remoteClient(remote.endpoint)[method]()).rejects.toThrow("Do not repeat the reset");
  ...
  it("does not accept completion from a redirected status route", async () => {
    ...
    const failure = remoteClient(remote.endpoint)[method]();
