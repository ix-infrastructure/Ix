...
#
# Usage:
#   ./scripts/connect.sh ~/my-project                  # Claude Code (default)
#   ./scripts/connect.sh ~/my-project --cursor         # Cursor
#   ./scripts/connect.sh ~/my-project --claude-desktop # Claude Desktop
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo ""
      echo "Options:"
      echo "  --claude-code        Install MCP for Claude Code (default)"
      echo "  --claude-desktop     Install MCP for Claude Desktop"
      echo "  --cursor             Install MCP for Cursor"
      echo "  --skip-ingest        Don't ingest the codebase yet"
      echo "  -h, --help           Show this help"
      echo ""
      echo "What this does:"
      echo "  1. Creates .mcp.json in the project (MCP server config)"
      echo "  2. Adds IX rules to CLAUDE.md (mandatory LLM workflow)"
      echo "  3. Ingests the project's source code into the knowledge graph"
      echo ""
...
# ── Step 2: Add IX rules to CLAUDE.md ────────────────────────────────────────
...
# ── Step 3: Ingest codebase ─────────────────────────────────────────────────

if [ "$SKIP_INGEST" = true ]; then
  echo "── [3] Ingest (skipped) ───────────────────────────"
else
  echo "── [3] Ingest Codebase ────────────────────────────"
  cd "$PROJECT_DIR"
...
echo ""
echo "  Next steps:"
echo "    1. Open $PROJECT_DIR in $IDE"
echo "    2. The IX MCP server starts automatically"
echo "    3. Ask Claude questions about your codebase"
echo ""
echo "  To disconnect:"
